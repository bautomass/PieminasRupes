// src/lib/translations.ts
export const translations = {
  lv: {
    hero: {
      title: "SkyGarden kapu kopšana",
      subtitle:
        "Profesionāla kapavietu kopšana, kas saglabā un godina Jūsu tuvinieku piemiņu",
      cta: "Pieteikt konsultāciju",
      secondaryLink: "Uzzināt vairāk",
      scrollText: "Apskatīt piedāvājumu",
    },
    services: {
      title: "Mūsu pakalpojumi",
      subtitle: "Visaptverošs kapu kopšanas serviss",
      regular: {
        title: "Regulāra kopšana",
        description:
          "Visaptveroša kapu vietas uzturēšana, kas ietver pavasara, vasaras un rudens darbus, lai kapavieta vienmēr izskatītos cienīgi.",
      },
      seasonal: {
        title: "Sezonālie darbi",
        description:
          "Kapu vietas sagatavošana atbilstoši sezonai - pavasara uzkopšana, vasaras kopšanas darbi, rudens lapu savākšana un ziemas ieziemošana.",
      },
      restoration: {
        title: "Pieminekļu atjaunošana",
        description:
          "Pieminekļu tīrīšana, kapu plākšņu atjaunošana un kapu vietas infrastruktūras labiekārtošana.",
      },
      custom: {
        title: "Individuāli risinājumi",
        description:
          "Pielāgoti pakalpojumi atbilstoši jūsu vēlmēm un vajadzībām kapu kopšanas jomā.",
      },
      learnMore: "UZZINĀT VAIRĀK",
      all: "Visi pakalpojumi",
      viewPricing: "Apskatīt visas cenas",
    },
    why: {
      title: "Kāpēc izvēlēties mūs?",
      subtitle: "Mēs rūpējamies par jūsu tuvinieku miera vietu",
      respect: {
        title: "Cieņa un pietāte",
        description:
          "Mēs strādājam ar dziļu cieņu pret jūsu aizgājušajiem tuviniekiem un viņu piemiņas vietām.",
      },
      quality: {
        title: "Augsta kvalitāte",
        description:
          "Izmantojam tikai kvalitatīvus materiālus un profesionālas metodes kapu kopšanā.",
      },
      individual: {
        title: "Individuāla pieeja",
        description:
          "Katrs klienta pieprasījums tiek uzklausīts un īstenots atbilstoši jūsu vēlmēm.",
      },
      experience: {
        title: "Pieredze un zināšanas",
        description:
          "Mūsu speciālisti ir apmācīti un pieredzējuši kapu kopšanas darbos.",
      },
    },
    memorial: {
      preTitle: "Mūsu pieeja",
      title: "Kā mēs varam palīdzēt jums godināt piemiņu",
      subtitle:
        "Piedāvājam profesionālus kapu kopšanas pakalpojumus, lai saglabātu jūsu tuvinieku piemiņas vietas cieņpilnā stāvoklī gadu gaitā",
      valuesTitle: "Mūsu vērtības",
      imageAlt: "Sakārtota piemiņas vieta",
      imageCaption:
        "Mēs rūpējamies par katru sīkumu, lai piemiņas vieta būtu cienīga",
      ctaTitle: "Vai vēlaties nodrošināt pienācīgu kapu vietas kopšanu?",
      ctaText:
        "Sazinieties ar mums, lai iegūtu personalizētu kapu kopšanas plānu.",
      ctaButton: "Sazināties ar mums",
      serviceArea: "Savus pakalpojumus sniedzam:",
      serviceAreas: ["visā Dienvidkurzemes novadā", "Liepājā"],
      orderTitle: "Kā pasūtīt mūsu pakalpojumus",
      orderText:
        "Pakalpojuma pasūtīšanai, lūdzam norādīt aizgājēja informāciju - pilnu vārdu un uzvārdu, miršanas datumu un gadu, kā arī kapsētas nosaukumu. Ja zināt kapuvietas numuru vai rindu, tas ievērojami atvieglos kapavietas atrašanu.",
      orderExtra:
        "Ja iespējams, lūdzam pievienot fotogrāfiju vai aprakstu par pieminekli vai citām atpazīšanas zīmēm, kas atrodas kapavietā. Tas palīdzēs mums precīzāk identificēt konkrēto vietu.",
      graveSearchTitle: "Kapavietas atrašanas pakalpojums",
      graveSearchText:
        "Ja ilgāku laiku neesat apmeklējuši tuvinieka kapavietu un zināt tikai vārdu, miršanas datumu un kapsētu, mēs varam palīdzēt ar kapavietas atrašanas un stāvokļa novērtēšanas pakalpojumu. Mūsu speciālisti apzinīgi meklēs un dokumentēs situāciju.",
      graveSearchNote:
        "Lūdzu, ņemiet vērā, ka kapavietas meklēšana ar ierobežotu informāciju ir specializēts pakalpojums. Tā cena tiek noteikta individuāli, atkarībā no nepieciešamās izpētes apjoma, lai atrastu meklēto kapavietu.",
    },
    pricingBanner: {
      title: "Mūsu cenu piedāvājums",
      description:
        "Piedāvājam profesionālus un kvalitatīvus kapu kopšanas pakalpojumus par caurspīdīgām un konkurētspējīgām cenām, pielāgotus dažādiem kapavietu izmēriem.",
      smallSize: "Maza (līdz 7,5m²)",
      mediumSize: "Vidēja (7,5-9m²)",
      largeSize: "Liela (9-15m²)",
      veryLargeSize: "Ļoti liela (15-30m²)",
      ctaTitle: "Uzzināt vairāk par mūsu cenām",
      ctaText: "Apskatiet visus mūsu pakalpojumus un individuālos piedāvājumus",
      buttonText: "Apskatīt visas cenas",
    },
    process: {
      title: "Mūsu darba process",
      subtitle: "Vienkāršs process, vislabākais rezultāts",
      steps: [
        {
          title: "Konsultācija",
          description:
            "Sazināmies, lai saprastu jūsu vēlmes par piemiņas vietas kopšanu un iekārtošanu.",
        },
        {
          title: "Piedāvājums",
          description:
            "Sagatavojam individuālu risinājumu, kas atbilst jūsu prasībām un budžetam.",
        },
        {
          title: "Darbu izpilde",
          description:
            "Veicam kapu kopšanas darbus ar rūpību, pietāti un profesionalitāti.",
        },
        {
          title: "Atskaite",
          description:
            "Nosūtām detalizētu atskaiti ar fotogrāfijām pirms un pēc darbu veikšanas.",
        },
      ],
    },
    cta: {
      title: "Pieteikt konsultāciju",
      description:
        "Mēs palīdzēsim jums nodrošināt cienīgu piemiņu jūsu tuviniekiem",
      button: "Sazināties ar mums",
      or: "vai",
      call: "Zvaniet mums: +371 2X XXX XXX",
    },
    serviceDetails: {
      regular: {
        title: "Regulāra kopšana",
        description:
          "Visaptveroša kapu vietas uzturēšana, lai tā vienmēr būtu tīra un kārtīga.",
        includes: "Ietver sevī",
        services: [
          {
            title:
              "Pavasara lielo uzkopšanu (aprīļa sākums līdz aprīļa 3 dekādei)",
            items: [
              "Teritorijas tīrīšanu",
              "Teritorijas pārrakšanu (dziļo ravēšanu ar nezāļu sakņu izņemšanu)",
              "Teritorijas izlīmeņošanu",
              "Svaigu smilšu, melnzemes un mulčas kārtas uzbēršanu",
              "Granīta šķembu seguma izpūšanu ar lapu pūtēju",
              "Dekoratīvo olīšu mazgāšanu (kapu rāmīšos)",
              "Krūmu aprakšanu, griešanu",
              "Kapu pieminekļu un kapu rāmīšu, apmaļu, soliņu tīrīšanu",
              "Ilggadīgo augu pārstādīšanu vai apkopšanu",
            ],
          },
          {
            title:
              "Vasaras uzkopšanas 2 reizes (maijs/jūnijs un jūlijs/augusts)",
            items: [
              "Teritorijas nogrābšanu",
              "Ravēšanu",
              "Sauso zaru, skuju un citu gružu novākšanu",
              "Vasaras puķu stādīšanu un kopšanu",
            ],
          },
          {
            title:
              "Rudens uzkopšanu (oktobra beigas/novembra vidus līdz Svecīšu vakaram)",
            items: [
              "Rudens lapu grābšanu",
              "Teritorijas tīrīšanu",
              "Pāraugušo vai nokaltušo vasaras augu novākšanu",
              "Egļu zaru noklāšanu",
              "Svecīšu iedegšanu",
            ],
          },
        ],
        note: "Pēc klienta vēlēšanās, iespējams pievienot biežāku kapavietas apmeklējumu skaitu, lai apkoptu/aplaistītu stādījumus. Jebkura papildus reizes izmaksas tiek saskaņotas individuāli.",
      },
      seasonal: {
        title: "Sezonālie darbi",
        description:
          "Kapu vietas sakopšana un dekorēšana atbilstoši sezonai un svētkiem.",
        seasons: [
          {
            title: "Pavasaris",
            description:
              "Pirmā pavasara uzkopšana sākas ar sausās zāles, lapu, skuju un gružu savākšanu. Pieminekļa, apmalīšu un pārējo elementu mitro tīrīšanu. Pēc uzkopšanas un tīrīšanas tiek veikta stādu stādīšana tiem paredzētajās vietās.",
          },
          {
            title: "Vasara",
            description:
              "Kārtības uzturēšanai vasarā tiek apgriezti krūmi, tiek veikta pieminekļa, apmalīšu un pārējo elementu mitrā tīrīšana, nezāļu ravēšana visā teritorijā. Sausā laikā apstādījumu laistīšana.",
          },
          {
            title: "Rudens",
            description:
              "Pirms ziemas nepieciešama rudens lapu savākšana, viengadīgo augu izrakšana. Ieziemošani izmantojam skujeņu zarus ar kuriem noklājam visu kapa kopiņu.",
          },
        ],
      },
      pricing: {
        title: "Cenas atkarīgas no kapavietas izmēra",
        quarterly: "reizi 3 mēnešos",
        yearly: "gadā",
        paymentNote:
          "Jāmaksā 50% no kopējās summas pirms darbu uzsākšanas. Atlikums jāmaksā pēc darbu pabeigšanas.",
        additionalNote:
          "Klienti var pievienot papildu pakalpojumus par papildu maksu, atkarībā no produkta cenas.",
        sizes: [
          {
            title: "Maza",
            size: "līdz 7,5m²",
            price: "35",
            yearlyPrice: "315",
          },
          {
            title: "Vidēja",
            size: "7,5 - 9m²",
            price: "45",
            yearlyPrice: "405",
          },
          {
            title: "Liela",
            size: "9 - 15m²",
            price: "50",
            yearlyPrice: "450",
          },
          {
            title: "Ļoti liela",
            size: "15 - 30m²",
            price: "60",
            yearlyPrice: "540",
          },
        ],
        note: "* Transports NAV iekļauts cenā",
        subscriptionBadge: "Abonements",
        oneTimeBadge: "Vienreizējs pakalpojums",
        subscriptionNote: "Regulāra apkope ar abonementu",
        oneTimeNote: "Cena par vienreizēju pakalpojumu",
      },
      packages: {
        title: "Kapu kopšanas pakalpojumu paketes",
        subtitle: "Izvēlieties piemērotāko paketi atbilstoši jūsu vajadzībām",
        standard: {
          title: "Standarta",
          price: "34",
          yearlyPrice: "374",
          description: "Visaptverošs pamata kopšanas pakalpojums",
          dressing: "Kapa vietas dekorēšana šajos gadījumos:",
          features: [
            "Ziemassvētki",
            "Lieldienas",
            "Kapu svētki",
            "Nāves gadadienas",
          ],
        },
        plus: {
          title: "Plus",
          price: "37",
          yearlyPrice: "407",
          description: "Paplašināts kopšanas pakalpojums ar papildu dekorēšanu",
          dressing: "Kapa vietas dekorēšana šajos gadījumos:",
          features: [
            "Ziemassvētki",
            "Lieldienas",
            "Kapu svētki",
            "Nāves gadadienas",
            "Mātes diena un Tēva diena",
          ],
        },
        premium: {
          title: "Premium",
          price: "44",
          yearlyPrice: "484",
          description:
            "Pilnīga kapu vietas kopšana ar visiem dekorēšanas pakalpojumiem",
          dressing: "Kapa vietas dekorēšana šajos gadījumos:",
          features: [
            "Ziemassvētki",
            "Lieldienas",
            "Kapu svētki",
            "Nāves gadadienas",
            "Mātes diena un Tēva diena",
            "Dzimšanas dienas",
            "Kāzu gadadienas",
            "Valentīndiena",
            "Visu dvēseļu diena",
          ],
        },
      },
      additionalServices: {
        title: "Papildus pakalpojumi",
        monthsMind: {
          title: "Mēneša piemiņas pakalpojums",
          description:
            "Mēs saprotam, cik grūts un emocionāls ir šis laiks, tāpēc piedāvājam mēneša piemiņas kapu kopšanas pakalpojumu. Tas tiek veikts pirms jūsu mīļā cilvēka piemiņas mēneša. Tas ietver akmeņu, veco ziedu noņemšanu, kapa sakārtošanu un pārklāšanu ar jūsu izvēlētu sūnu kūdru, augsni vai šķembām. Visbeidzot, mēs piegādājam un novietojam svaigi grieztu ziedu izlasi piemiņas vāzē.",
        },
        graveTending: {
          title: "Kapu kopšanas pakalpojums ietver",
          features: [
            "Zāles apgriešanu ap kapakmeni un kapa laukumu",
            "Nezāļu noņemšanu",
            "Vieglu kapakmeņa, podu un ziedu turētāju mazgāšanu",
            "Vieglu ēriku, krūmu, mazo koku un citu augu apgriešanu, ja nepieciešams",
            "Visu gružu - veco ziedu, augu un lapu u.c. noņemšanu",
            "Sezonālu kapu krūmu, sīpolu un jūsu izvēlēto augu stādīšanu",
          ],
        },
        yearlyServices: {
          title: "Kapu ikgadējie pakalpojumi",
          description:
            "Jūs varat izlemt, cik bieži vēlaties, lai jūsu mīļā cilvēka kaps tiktu uzturēts. Jūs varat izvēlēties no šādiem kapu kopšanas pakalpojumiem:",
          options: [
            "Reizi mēnesī",
            "Reizi divos mēnešos",
            "Reizi ceturksnī",
            "Reizi 6 mēnešos",
            "Reizi gadā",
          ],
        },
        floralDelivery: {
          title: "Ziedu piegādes pakalpojums",
          description:
            "Mēs nodrošinām svaigu ziedu vai vainagu piegādi un novietošanu. Jūs varat izvēlēties, lai tie tiktu novietoti katrā apmeklējumā vai jūsu izvēlētās īpašās dienās, lai atzīmētu gadadienas, dzimšanas dienas, mātes vai tēva dienu, kapu svētkus. Ziemassvētku rotājumi tiek piegādāti dienu vai divas pirms Ziemassvētku dienas.",
          photoNote:
            "Mēs fotografējam kapa laukumu pirms un pēc kapu kopšanas pakalpojuma. Jūs saņemsiet šīs fotogrāfijas pa pastu vai e-pastu.",
        },
        additional: {
          title: "Papildu pakalpojumi",
          features: [
            "Kapa šķembu tīrīšana un atjaunošana",
            "Sūnu kūdras vai augsnes atjaunošana",
            "Betona pamats, pārklāts ar jūsu izvēlētām kapa šķembām",
            "Specializēta kapakmeņa un apkārtnes tīrīšana – cena pēc apskates",
            "Kapa pieminekļa restaurācija – cena pēc apskates",
            "Epitāfa uzraksta atjaunošana – cena pēc apskates",
          ],
        },
      },
    },
  },
  en: {
    hero: {
      title: "SkyGarden Memorial Care",
      subtitle:
        "Professional care that preserves and honors the memory of your loved ones",
      cta: "Request Consultation",
      secondaryLink: "Learn More",
      scrollText: "View Our Services",
    },
    services: {
      title: "Our Services",
      subtitle: "Comprehensive grave care service",
      regular: {
        title: "Regular Maintenance",
        description:
          "Comprehensive grave site maintenance including spring, summer, and autumn work to ensure your memorial site always looks dignified.",
      },
      seasonal: {
        title: "Seasonal Work",
        description:
          "Grave site preparation according to the season - spring cleaning, summer maintenance, autumn leaf collection, and winter preparation.",
      },
      restoration: {
        title: "Monument Restoration",
        description:
          "Monument cleaning, grave plate restoration, and grave site infrastructure improvement.",
      },
      custom: {
        title: "Custom Solutions",
        description:
          "Tailored services according to your wishes and needs in grave maintenance.",
      },
      learnMore: "LEARN MORE",
      all: "All Services",
      viewPricing: "View All Prices",
    },
    why: {
      title: "Why Choose Us?",
      subtitle: "We care for your loved ones' resting place",
      respect: {
        title: "Respect and Reverence",
        description:
          "We work with deep respect for your departed loved ones and their memorial sites.",
      },
      quality: {
        title: "High Quality",
        description:
          "We use only quality materials and professional methods in grave care.",
      },
      individual: {
        title: "Individual Approach",
        description:
          "Every client request is heard and implemented according to your wishes.",
      },
      experience: {
        title: "Experience and Knowledge",
        description:
          "Our specialists are trained and experienced in grave maintenance work.",
      },
    },
    memorial: {
      preTitle: "Our Approach",
      title: "How We Can Help You Honor Memories",
      subtitle:
        "We offer professional grave maintenance services to keep your loved ones' memorial sites in dignified condition through the years",
      valuesTitle: "Our Values",
      imageAlt: "Well-maintained memorial site",
      imageCaption:
        "We care for every detail to ensure the memorial site remains dignified",
      ctaTitle: "Would you like to ensure proper memorial site care?",
      ctaText: "Contact us for a personalized grave maintenance plan.",
      ctaButton: "Contact Us",
      serviceArea: "We provide our services in:",
      serviceAreas: ["the entire Dienvidkurzeme region", "Liepāja"],
      orderTitle: "How to order our service",
      orderText:
        "To order our service, please provide details about the deceased - full name, date and year of death, and cemetery name. If you know the plot location or row number, this will greatly help us locate the grave site more efficiently.",
      orderExtra:
        "If possible, please include a photograph or description of any monument or distinctive markers at the grave site. This will assist us in precisely identifying the specific location.",
      graveSearchTitle: "Grave location service",
      graveSearchText:
        "If you haven't visited a loved one's grave for an extended period and only know their name, date of death, and cemetery, we can assist with our grave location and condition assessment service. Our specialists will diligently search and document the situation.",
      graveSearchNote:
        "Please note that locating graves with limited information is a specialized service. Pricing is determined individually, based on the extent of research required to find the grave you're looking for.",
    },
    pricingBanner: {
      title: "Our Pricing Options",
      description:
        "We offer professional and high-quality grave care services at transparent and competitive prices, tailored to various grave site sizes.",
      smallSize: "Small (up to 7.5m²)",
      mediumSize: "Medium (7.5-9m²)",
      largeSize: "Large (9-15m²)",
      veryLargeSize: "Very large (15-30m²)",
      ctaTitle: "Learn more about our pricing",
      ctaText: "Explore all our services and individual offerings",
      buttonText: "View all pricing",
    },
    process: {
      title: "Our Work Process",
      subtitle: "Simple process, best result",
      steps: [
        {
          title: "Consultation",
          description:
            "We get in touch to understand your wishes about memorial site maintenance and arrangements.",
        },
        {
          title: "Offer",
          description:
            "We prepare an individual solution that meets your requirements and budget.",
        },
        {
          title: "Execution",
          description:
            "We perform grave maintenance work with care, reverence, and professionalism.",
        },
        {
          title: "Report",
          description:
            "We send a detailed report with before and after photos of the completed work.",
        },
      ],
    },
    cta: {
      title: "Request a Consultation",
      description:
        "We will help you ensure a dignified memory for your loved ones",
      button: "Contact Us",
      or: "or",
      call: "Call us: +371 2X XXX XXX",
    },
    serviceDetails: {
      regular: {
        title: "Regular Maintenance",
        description:
          "Comprehensive grave site maintenance to keep it clean and tidy at all times.",
        includes: "Includes",
        services: [
          {
            title: "Major spring cleaning (early April to late April)",
            items: [
              "Territory cleaning",
              "Deep soil cultivation (deep weeding with removal of weed roots)",
              "Territory leveling",
              "Fresh sand, soil, and mulch application",
              "Granite gravel cleaning with leaf blower",
              "Decorative pebble washing (in grave frames)",
              "Bush trimming and maintenance",
              "Cleaning of monuments, grave frames, borders, and benches",
              "Transplanting or maintenance of perennial plants",
            ],
          },
          {
            title: "Summer maintenance 2 times (May/June and July/August)",
            items: [
              "Territory raking",
              "Weeding",
              "Removal of dry branches, needles, and other debris",
              "Planting and care of summer flowers",
            ],
          },
          {
            title:
              "Autumn cleaning (late October/mid-November until Candle Evening)",
            items: [
              "Autumn leaf raking",
              "Territory cleaning",
              "Removal of overgrown or dried summer plants",
              "Covering with fir branches",
              "Candle lighting",
            ],
          },
        ],
        note: "At the client's request, it is possible to add more frequent grave site visits to maintain/water the plantings. The cost of any additional visits is agreed upon individually.",
      },
      seasonal: {
        title: "Seasonal Work",
        description:
          "Grave site cleaning and decoration according to the season and holidays.",
        seasons: [
          {
            title: "Spring",
            description:
              "The first spring cleaning begins with the collection of dry grass, leaves, needles, and debris. Wet cleaning of the monument, borders, and other elements. After cleaning, plants are planted in their designated places.",
          },
          {
            title: "Summer",
            description:
              "For summer maintenance, bushes are trimmed, monument, borders, and other elements are wet cleaned, and weeds are removed throughout the territory. Watering of plantings during dry periods.",
          },
          {
            title: "Autumn",
            description:
              "Before winter, autumn leaves need to be collected and annual plants removed. For wintering, we use conifer branches to cover the entire grave mound.",
          },
        ],
      },
      pricing: {
        title: "Prices depend on grave site size",
        quarterly: "per quarter",
        yearly: "yearly",
        paymentNote:
          "50% of the total amount is due before work begins. The remainder is to be paid upon completion of the work.",
        additionalNote:
          "Customers can add additional services at extra cost, depending on the product price.",
        sizes: [
          {
            title: "Small",
            size: "up to 7.5m²",
            price: "35",
            yearlyPrice: "315",
          },
          {
            title: "Medium",
            size: "7.5 - 9m²",
            price: "45",
            yearlyPrice: "405",
          },
          {
            title: "Large",
            size: "9 - 15m²",
            price: "50",
            yearlyPrice: "450",
          },
          {
            title: "Very large",
            size: "15 - 30m²",
            price: "60",
            yearlyPrice: "540",
          },
        ],
        note: "* Transportation is NOT included in the price",
        subscriptionBadge: "Subscription",
        oneTimeBadge: "One-time Service",
        subscriptionNote: "Regular maintenance with subscription",
        oneTimeNote: "Price for one-time service",
      },
      packages: {
        title: "Grave Care Service Packages",
        subtitle: "Choose the most suitable package according to your needs",
        standard: {
          title: "Standard",
          price: "34",
          yearlyPrice: "374",
          description: "Comprehensive basic care service",
          dressing: "Grave is dressed for:",
          features: [
            "Christmas",
            "Easter",
            "Cemetery Mass",
            "Anniversaries of Death",
          ],
        },
        plus: {
          title: "Plus",
          price: "37",
          yearlyPrice: "407",
          description: "Extended care service with additional decorations",
          dressing: "Grave is dressed for:",
          features: [
            "Christmas",
            "Easter",
            "Cemetery Mass",
            "Anniversaries of Death",
            "Mother's Day & Father's Day",
          ],
        },
        premium: {
          title: "Premium",
          price: "44",
          yearlyPrice: "484",
          description: "Complete grave care with all decoration services",
          dressing: "Grave is dressed for:",
          features: [
            "Christmas",
            "Easter",
            "Cemetery Mass",
            "Anniversaries of Death",
            "Mother's Day & Father's Day",
            "Birthdays",
            "Wedding Anniversary",
            "Valentine's Day",
            "All Souls Day",
          ],
        },
      },
      additionalServices: {
        title: "Additional Services",
        monthsMind: {
          title: "Month's Mind Service",
          description:
            "We understand how difficult and emotional this time is so we offer a month's mind grave tending service. It is carried out prior to your loved one's month's mind. It involves removal of stones, old flowers, tidying of the grave and topping it with a choice of moss peat, top soil or chippings. Lastly, we supply and place a selection of freshly cut flowers in a memorial vase.",
        },
        graveTending: {
          title: "Grave tending service involves",
          features: [
            "Trimming of grass around the headstone and grave plot",
            "Removal of weeds",
            "Light wash of headstone, pots and flower holders",
            "Light pruning of heathers, shrubs, small trees and other plants if required",
            "Removal of all debris – old flowers, plants, and leaves etc.",
            "Seasonal planting of graves shrubs, bulbs and plants of your choice",
          ],
        },
        yearlyServices: {
          title: "Grave Yearly Services",
          description:
            "You can decide how often you would like your loved one's grave to be maintained. You can choose from the following grave tending services:",
          options: [
            "Monthly",
            "Bi-monthly",
            "Quarterly",
            "Every 6 months",
            "Yearly",
          ],
        },
        floralDelivery: {
          title: "Floral Delivery Service",
          description:
            "We provide the supply, delivery, and placing of fresh flowers or wreath tributes. You can choose to have them placed on each visit, or on special days of your choice to acknowledge anniversaries, birthdays, mother's or father's day, cemetery Sunday. Christmas tributes are delivered a day or so before Christmas Day.",
          photoNote:
            "We photograph the grave plot before and after our tending service. You will receive these photographs by post or e-mail.",
        },
        additional: {
          title: "Additional services",
          features: [
            "Cleaning and renewal of grave chippings",
            "Renewal of moss peat or topsoil",
            "Concrete base covered with grave chippings of your choice",
            "Specialist cleaning of headstone and surround – price on inspection",
            "Grave headstone restoration – price on inspection",
            "Re-gilding of epitaph inscription – price on inspection",
          ],
        },
      },
    },
  },
  ru: {
    hero: {
      title: "SkyGarden уход за могилами",
      subtitle:
        "Профессиональный уход, который сохраняет и чтит память о ваших близких",
      cta: "Запросить консультацию",
      secondaryLink: "Узнать больше",
      scrollText: "Посмотреть услуги",
    },
    services: {
      title: "Наши услуги",
      subtitle: "Комплексный уход за могилами",
      regular: {
        title: "Регулярный уход",
        description:
          "Комплексное обслуживание места захоронения, включающее весенние, летние и осенние работы, чтобы могила всегда выглядела достойно.",
      },
      seasonal: {
        title: "Сезонные работы",
        description:
          "Подготовка могилы в соответствии с сезоном - весенняя уборка, летний уход, осенний сбор листьев и зимняя подготовка.",
      },
      restoration: {
        title: "Реставрация памятников",
        description:
          "Чистка памятников, реставрация надгробных плит и улучшение инфраструктуры места захоронения.",
      },
      custom: {
        title: "Индивидуальные решения",
        description:
          "Индивидуальные услуги в соответствии с вашими пожеланиями и потребностями в сфере ухода за могилами.",
      },
      learnMore: "УЗНАТЬ БОЛЬШЕ",
      all: "Все услуги",
      viewPricing: "Посмотреть все цены",
    },
    why: {
      title: "Почему выбирают нас?",
      subtitle: "Мы заботимся о месте покоя ваших близких",
      respect: {
        title: "Уважение и почтение",
        description:
          "Мы работаем с глубоким уважением к вашим ушедшим близким и местам их памяти.",
      },
      quality: {
        title: "Высокое качество",
        description:
          "Мы используем только качественные материалы и профессиональные методы в уходе за могилами.",
      },
      individual: {
        title: "Индивидуальный подход",
        description:
          "Каждый запрос клиента выслушивается и реализуется в соответствии с вашими пожеланиями.",
      },
      experience: {
        title: "Опыт и знания",
        description:
          "Наши специалисты обучены и имеют опыт работы по уходу за могилами.",
      },
    },
    memorial: {
      preTitle: "Наш подход",
      title: "Как мы можем помочь вам почтить память",
      subtitle:
        "Мы предлагаем профессиональные услуги по уходу за могилами, чтобы поддерживать места памяти ваших близких в достойном состоянии на протяжении многих лет",
      valuesTitle: "Наши ценности",
      imageAlt: "Ухоженное место памяти",
      imageCaption:
        "Мы заботимся о каждой детали, чтобы место памяти оставалось достойным",
      ctaTitle: "Хотите обеспечить надлежащий уход за местом захоронения?",
      ctaText:
        "Свяжитесь с нами для получения индивидуального плана по уходу за могилой.",
      ctaButton: "Связаться с нами",
      serviceArea: "Мы предоставляем наши услуги в:",
      serviceAreas: ["по всему Южнокурземскому краю", "Лиепае"],
      orderTitle: "Как заказать нашу услугу",
      orderText:
        "Для заказа нашей услуги, пожалуйста, предоставьте данные об усопшем - полное имя, дату и год смерти, а также название кладбища. Если вы знаете расположение участка или номер ряда, это значительно поможет нам более эффективно найти могилу.",
      orderExtra:
        "По возможности, приложите фотографию или описание памятника или отличительных знаков на могиле. Это поможет нам точно идентифицировать конкретное место.",
      graveSearchTitle: "Услуга поиска могилы",
      graveSearchText:
        "Если вы не посещали могилу близкого человека в течение длительного времени и знаете только его имя, дату смерти и кладбище, мы можем помочь с нашей услугой по поиску могилы и оценке её состояния. Наши специалисты будут тщательно искать и документировать ситуацию.",
      graveSearchNote:
        "Обратите внимание, что поиск могил при ограниченной информации - это специализированная услуга. Цена определяется индивидуально, в зависимости от объема исследований, необходимых для поиска нужной вам могилы.",
    },
    pricingBanner: {
      title: "Наши ценовые предложения",
      description:
        "Мы предлагаем профессиональные и качественные услуги по уходу за могилами по прозрачным и конкурентоспособным ценам, адаптированные к различным размерам могил.",
      smallSize: "Маленькая (до 7,5м²)",
      mediumSize: "Средняя (7,5-9м²)",
      largeSize: "Большая (9-15м²)",
      veryLargeSize: "Очень большая (15-30м²)",
      ctaTitle: "Узнать больше о наших ценах",
      ctaText:
        "Ознакомьтесь со всеми нашими услугами и индивидуальными предложениями",
      buttonText: "Посмотреть все цены",
    },
    process: {
      title: "Наш рабочий процесс",
      subtitle: "Простой процесс, лучший результат",
      steps: [
        {
          title: "Консультация",
          description:
            "Мы связываемся с вами, чтобы понять ваши пожелания по уходу и обустройству места памяти.",
        },
        {
          title: "Предложение",
          description:
            "Мы готовим индивидуальное решение, которое соответствует вашим требованиям и бюджету.",
        },
        {
          title: "Выполнение",
          description:
            "Мы выполняем работы по уходу за могилами с заботой, почтением и профессионализмом.",
        },
        {
          title: "Отчет",
          description:
            "Мы отправляем подробный отчет с фотографиями до и после выполненных работ.",
        },
      ],
    },
    cta: {
      title: "Запросить консультацию",
      description:
        "Мы поможем вам обеспечить достойную память для ваших близких",
      button: "Связаться с нами",
      or: "или",
      call: "Позвоните нам: +371 2X XXX XXX",
    },
    serviceDetails: {
      regular: {
        title: "Регулярный уход",
        description:
          "Комплексное обслуживание могилы для поддержания её в чистоте и порядке.",
        includes: "Включает в себя",
        services: [
          {
            title: "Большая весенняя уборка (начало апреля до конца апреля)",
            items: [
              "Очистка территории",
              "Перекопка территории (глубокая прополка с удалением корней сорняков)",
              "Выравнивание территории",
              "Нанесение свежего песка, почвы и мульчи",
              "Очистка гранитного гравия с помощью воздуходувки",
              "Мытье декоративной гальки (в рамках могилы)",
              "Обрезка и уход за кустами",
              "Очистка памятников, рамок могил, бордюров и скамеек",
              "Пересадка или уход за многолетними растениями",
            ],
          },
          {
            title: "Летний уход 2 раза (май/июнь и июль/август)",
            items: [
              "Уборка территории",
              "Прополка",
              "Удаление сухих веток, хвои и прочего мусора",
              "Посадка и уход за летними цветами",
            ],
          },
          {
            title:
              "Осенняя уборка (конец октября/середина ноября до Вечера свечей)",
            items: [
              "Сгребание осенних листьев",
              "Очистка территории",
              "Удаление переросших или засохших летних растений",
              "Покрытие еловыми ветками",
              "Зажигание свечей",
            ],
          },
        ],
        note: "По желанию клиента возможно добавление более частых посещений могилы для ухода/полива насаждений. Стоимость любых дополнительных посещений согласовывается индивидуально.",
      },
      seasonal: {
        title: "Сезонные работы",
        description:
          "Уборка и украшение могилы в соответствии с сезоном и праздниками.",
        seasons: [
          {
            title: "Весна",
            description:
              "Первая весенняя уборка начинается со сбора сухой травы, листьев, хвои и мусора. Влажная очистка памятника, бордюров и других элементов. После уборки и очистки проводится посадка растений в отведенных для них местах.",
          },
          {
            title: "Лето",
            description:
              "Для поддержания порядка летом подстригаются кусты, проводится влажная очистка памятника, бордюров и других элементов, прополка сорняков по всей территории. В сухое время полив насаждений.",
          },
          {
            title: "Осень",
            description:
              "Перед зимой необходим сбор осенних листьев, выкапывание однолетних растений. Для зимовки используем хвойные ветки, которыми покрываем весь могильный холмик.",
          },
        ],
      },
      pricing: {
        title: "Цены зависят от размера могилы",
        quarterly: "в квартал",
        yearly: "в год",
        paymentNote:
          "50% от общей суммы оплачивается до начала работ. Оставшаяся сумма оплачивается после завершения работ.",
        additionalNote:
          "Клиенты могут добавить дополнительные услуги за дополнительную плату, в зависимости от цены продукта.",
        sizes: [
          {
            title: "Маленькая",
            size: "до 7,5м²",
            price: "35",
            yearlyPrice: "315",
          },
          {
            title: "Средняя",
            size: "7,5 - 9м²",
            price: "45",
            yearlyPrice: "405",
          },
          {
            title: "Большая",
            size: "9 - 15м²",
            price: "50",
            yearlyPrice: "450",
          },
          {
            title: "Очень большая",
            size: "15 - 30м²",
            price: "60",
            yearlyPrice: "540",
          },
        ],
        note: "* Транспорт НЕ включен в цену",
        subscriptionBadge: "Абонемент",
        oneTimeBadge: "Разовая услуга",
        subscriptionNote: "Регулярное обслуживание по подписке",
        oneTimeNote: "Цена за разовую услугу",
      },
      packages: {
        title: "Пакеты услуг по уходу за могилами",
        subtitle:
          "Выберите наиболее подходящий пакет в соответствии с вашими потребностями",
        standard: {
          title: "Стандарт",
          price: "34",
          yearlyPrice: "374",
          description: "Комплексный базовый уход",
          dressing: "Могила украшается для:",
          features: [
            "Рождества",
            "Пасхи",
            "Кладбищенской мессы",
            "Годовщин смерти",
          ],
        },
        plus: {
          title: "Плюс",
          price: "37",
          yearlyPrice: "407",
          description: "Расширенный уход с дополнительными украшениями",
          dressing: "Могила украшается для:",
          features: [
            "Рождества",
            "Пасхи",
            "Кладбищенской мессы",
            "Годовщин смерти",
            "Дня матери и Дня отца",
          ],
        },
        premium: {
          title: "Премиум",
          price: "44",
          yearlyPrice: "484",
          description: "Полный уход за могилой со всеми услугами по украшению",
          dressing: "Могила украшается для:",
          features: [
            "Рождества",
            "Пасхи",
            "Кладбищенской мессы",
            "Годовщин смерти",
            "Дня матери и Дня отца",
            "Дней рождения",
            "Годовщин свадьбы",
            "Дня Святого Валентина",
            "Дня всех душ",
          ],
        },
      },
      additionalServices: {
        title: "Дополнительные услуги",
        monthsMind: {
          title: "Месячный поминальный сервис",
          description:
            "Мы понимаем, насколько трудным и эмоциональным является это время, поэтому предлагаем месячный поминальный сервис по уходу за могилой. Он проводится перед месячной памятью вашего близкого. Включает в себя удаление камней, старых цветов, уборку могилы и покрытие её на ваш выбор торфяным мхом, верхним слоем почвы или щебнем. В завершение мы поставляем и размещаем набор свежесрезанных цветов в мемориальной вазе.",
        },
        graveTending: {
          title: "Уход за могилой включает",
          features: [
            "Подстригание травы вокруг надгробия и могильного участка",
            "Удаление сорняков",
            "Легкое мытье надгробия, горшков и держателей для цветов",
            "Легкую обрезку вереска, кустарников, небольших деревьев и других растений при необходимости",
            "Удаление всего мусора – старых цветов, растений, листьев и т.д.",
            "Сезонную посадку кустарников, луковиц и растений на могиле по вашему выбору",
          ],
        },
        yearlyServices: {
          title: "Годовые услуги по уходу за могилой",
          description:
            "Вы можете решить, как часто вы хотите, чтобы за могилой вашего близкого ухаживали. Вы можете выбрать из следующих услуг по уходу за могилой:",
          options: [
            "Ежемесячно",
            "Раз в два месяца",
            "Ежеквартально",
            "Раз в 6 месяцев",
            "Ежегодно",
          ],
        },
        floralDelivery: {
          title: "Услуга доставки цветов",
          description:
            "Мы обеспечиваем поставку, доставку и размещение свежих цветов или венков. Вы можете выбрать их размещение при каждом посещении или в особые дни по вашему выбору, чтобы отметить годовщины, дни рождения, день матери или отца, кладбищенское воскресенье. Рождественские венки доставляются за день или два до Рождества.",
          photoNote:
            "Мы фотографируем могильный участок до и после нашего ухода. Вы получите эти фотографии по почте или по электронной почте.",
        },
        additional: {
          title: "Дополнительные услуги",
          features: [
            "Чистка и обновление могильной щебенки",
            "Обновление торфяного мха или верхнего слоя почвы",
            "Бетонное основание, покрытое могильной щебенкой по вашему выбору",
            "Специализированная чистка надгробия и окружения – цена по осмотру",
            "Реставрация надгробия – цена по осмотру",
            "Обновление позолоты эпитафической надписи – цена по осмотру",
          ],
        },
      },
    },
  },
  de: {
    hero: {
      title: "SkyGarden Grabpflege",
      subtitle:
        "Professionelle Pflege, die das Andenken an Ihre Lieben bewahrt und ehrt",
      cta: "Beratung anfragen",
      secondaryLink: "Mehr erfahren",
      scrollText: "Dienstleistungen ansehen",
    },
    services: {
      title: "Unsere Dienstleistungen",
      subtitle: "Umfassender Grabpflegeservice",
      regular: {
        title: "Regelmäßige Pflege",
        description:
          "Umfassende Grabpflege mit Frühlings-, Sommer- und Herbstarbeiten, damit die Grabstätte immer würdevoll aussieht.",
      },
      seasonal: {
        title: "Saisonale Arbeiten",
        description:
          "Vorbereitung der Grabstätte entsprechend der Jahreszeit - Frühjahrsreinigung, Sommerpflege, Herbstlaubsammlung und Wintervorbereitung.",
      },
      restoration: {
        title: "Denkmalrestaurierung",
        description:
          "Denkmalsreinigung, Grabplattenrestaurierung und Verbesserung der Grabinfrastruktur.",
      },
      custom: {
        title: "Individuelle Lösungen",
        description:
          "Maßgeschneiderte Dienstleistungen nach Ihren Wünschen und Bedürfnissen im Bereich der Grabpflege.",
      },
      learnMore: "MEHR ERFAHREN",
      all: "Alle Dienstleistungen",
      viewPricing: "Alle Preise ansehen",
    },
    why: {
      title: "Warum uns wählen?",
      subtitle: "Wir kümmern uns um die Ruhestätte Ihrer Lieben",
      respect: {
        title: "Respekt und Ehrfurcht",
        description:
          "Wir arbeiten mit tiefem Respekt für Ihre verstorbenen Angehörigen und ihre Gedenkstätten.",
      },
      quality: {
        title: "Hohe Qualität",
        description:
          "Wir verwenden nur hochwertige Materialien und professionelle Methoden bei der Grabpflege.",
      },
      individual: {
        title: "Individueller Ansatz",
        description:
          "Jede Kundenanfrage wird gehört und nach Ihren Wünschen umgesetzt.",
      },
      experience: {
        title: "Erfahrung und Wissen",
        description:
          "Unsere Spezialisten sind ausgebildet und erfahren in der Grabpflege.",
      },
    },
    memorial: {
      preTitle: "Unser Ansatz",
      title: "Wie wir Ihnen helfen können, Erinnerungen zu ehren",
      subtitle:
        "Wir bieten professionelle Grabpflegedienste an, um die Gedenkstätten Ihrer Lieben im Laufe der Jahre in einem würdigen Zustand zu erhalten",
      valuesTitle: "Unsere Werte",
      imageAlt: "Gepflegte Gedenkstätte",
      imageCaption:
        "Wir kümmern uns um jedes Detail, damit die Gedenkstätte würdevoll bleibt",
      ctaTitle: "Möchten Sie eine angemessene Grabpflege sicherstellen?",
      ctaText:
        "Kontaktieren Sie uns für einen personalisierten Grabpflegeplan.",
      ctaButton: "Kontakt aufnehmen",
      serviceArea: "Wir bieten unsere Dienstleistungen an in:",
      serviceAreas: ["der gesamten Region Dienvidkurzeme", "Liepāja"],
      orderTitle: "So bestellen Sie unseren Service",
      orderText:
        "Um unseren Service zu bestellen, geben Sie bitte Informationen über den Verstorbenen an - vollständiger Name, Sterbedatum und -jahr sowie Name des Friedhofs. Wenn Sie den Grabstandort oder die Reihennummer kennen, hilft uns dies, das Grab effizienter zu finden.",
      orderExtra:
        "Wenn möglich, fügen Sie bitte ein Foto oder eine Beschreibung von Denkmälern oder markanten Merkmalen am Grab bei. Dies hilft uns bei der genauen Identifizierung des spezifischen Standorts.",
      graveSearchTitle: "Grabsuchdienst",
      graveSearchText:
        "Wenn Sie das Grab eines Angehörigen längere Zeit nicht besucht haben und nur den Namen, das Sterbedatum und den Friedhof kennen, können wir Ihnen mit unserem Grabsuch- und Zustandsbewertungsdienst helfen. Unsere Spezialisten werden sorgfältig suchen und die Situation dokumentieren.",
      graveSearchNote:
        "Bitte beachten Sie, dass die Suche nach Gräbern mit begrenzten Informationen ein spezialisierter Service ist. Die Preisgestaltung erfolgt individuell, basierend auf dem Umfang der Recherche, die erforderlich ist, um das gesuchte Grab zu finden.",
    },
    pricingBanner: {
      title: "Unsere Preisoptionen",
      description:
        "Wir bieten professionelle und hochwertige Grabpflegedienste zu transparenten und wettbewerbsfähigen Preisen an, abgestimmt auf verschiedene Grabgrößen.",
      smallSize: "Klein (bis zu 7,5m²)",
      mediumSize: "Mittel (7,5-9m²)",
      largeSize: "Groß (9-15m²)",
      veryLargeSize: "Sehr groß (15-30m²)",
      ctaTitle: "Mehr über unsere Preise erfahren",
      ctaText:
        "Entdecken Sie alle unsere Dienstleistungen und individuellen Angebote",
      buttonText: "Alle Preise ansehen",
    },
    process: {
      title: "Unser Arbeitsprozess",
      subtitle: "Einfacher Prozess, bestes Ergebnis",
      steps: [
        {
          title: "Beratung",
          description:
            "Wir nehmen Kontakt auf, um Ihre Wünsche zur Pflege und Gestaltung der Gedenkstätte zu verstehen.",
        },
        {
          title: "Angebot",
          description:
            "Wir erstellen eine individuelle Lösung, die Ihren Anforderungen und Ihrem Budget entspricht.",
        },
        {
          title: "Ausführung",
          description:
            "Wir führen Grabpflegearbeiten mit Sorgfalt, Ehrfurcht und Professionalität durch.",
        },
        {
          title: "Bericht",
          description:
            "Wir senden Ihnen einen detaillierten Bericht mit Vorher-Nachher-Fotos der erledigten Arbeiten.",
        },
      ],
    },
    cta: {
      title: "Beratung anfragen",
      description:
        "Wir helfen Ihnen, ein würdiges Andenken an Ihre Lieben zu bewahren",
      button: "Kontaktieren Sie uns",
      or: "oder",
      call: "Rufen Sie uns an: +371 2X XXX XXX",
    },
    serviceDetails: {
      regular: {
        title: "Regelmäßige Pflege",
        description:
          "Umfassende Grabpflege, um es jederzeit sauber und ordentlich zu halten.",
        includes: "Beinhaltet",
        services: [
          {
            title: "Große Frühjahrsreinigung (Anfang April bis Ende April)",
            items: [
              "Gebietsreinigung",
              "Tiefenkultivierung des Bodens (tiefes Jäten mit Entfernung von Unkrautwurzeln)",
              "Gebietsplanung",
              "Auftragen von frischem Sand, Erde und Mulch",
              "Reinigung von Granitkies mit Laubbläser",
              "Waschen von dekorativen Kieseln (in Grabeinfassungen)",
              "Beschneiden und Pflege von Büschen",
              "Reinigung von Denkmälern, Grabeinfassungen, Grenzen und Bänken",
              "Umpflanzen oder Pflege von mehrjährigen Pflanzen",
            ],
          },
          {
            title: "Sommerpflege 2 Mal (Mai/Juni und Juli/August)",
            items: [
              "Rechen des Gebiets",
              "Jäten",
              "Entfernung von trockenen Ästen, Nadeln und anderem Unrat",
              "Pflanzen und Pflege von Sommerblumen",
            ],
          },
          {
            title:
              "Herbstreinigung (Ende Oktober/Mitte November bis zum Kerzenabend)",
            items: [
              "Rechen von Herbstlaub",
              "Gebietsreinigung",
              "Entfernung überwucherter oder getrockneter Sommerpflanzen",
              "Abdeckung mit Tannenzweigen",
              "Kerzenanzünden",
            ],
          },
        ],
        note: "Auf Wunsch des Kunden ist es möglich, häufigere Besuche der Grabstätte hinzuzufügen, um die Bepflanzung zu pflegen/zu gießen. Die Kosten für zusätzliche Besuche werden individuell vereinbart.",
      },
      seasonal: {
        title: "Saisonale Arbeiten",
        description:
          "Grabreinigung und -dekoration entsprechend der Jahreszeit und den Feiertagen.",
        seasons: [
          {
            title: "Frühling",
            description:
              "Die erste Frühjahrsreinigung beginnt mit dem Sammeln von trockenem Gras, Blättern, Nadeln und Unrat. Nassreinigung des Denkmals, der Ränder und anderer Elemente. Nach der Reinigung werden Pflanzen an den vorgesehenen Stellen gepflanzt.",
          },
          {
            title: "Sommer",
            description:
              "Für die Sommerpflege werden Büsche beschnitten, Denkmal, Randsteine und andere Elemente nass gereinigt und Unkraut im gesamten Gebiet entfernt. Bewässerung der Bepflanzung in Trockenzeiten.",
          },
          {
            title: "Herbst",
            description:
              "Vor dem Winter müssen Herbstblätter gesammelt und einjährige Pflanzen entfernt werden. Für die Überwinterung verwenden wir Koniferenzweige, um den gesamten Grabhügel zu bedecken.",
          },
        ],
      },
      pricing: {
        title: "Preise abhängig von der Grabstättengröße",
        quarterly: "pro Quartal",
        yearly: "jährlich",
        paymentNote:
          "50% der Gesamtsumme sind vor Beginn der Arbeiten zu zahlen. Der Rest ist nach Abschluss der Arbeiten zu zahlen.",
        additionalNote:
          "Kunden können zusätzliche Leistungen gegen Aufpreis hinzufügen, abhängig vom Produktpreis.",
        sizes: [
          {
            title: "Klein",
            size: "bis zu 7,5m²",
            price: "35",
            yearlyPrice: "315",
          },
          {
            title: "Mittel",
            size: "7,5 - 9m²",
            price: "45",
            yearlyPrice: "405",
          },
          {
            title: "Groß",
            size: "9 - 15m²",
            price: "50",
            yearlyPrice: "450",
          },
          {
            title: "Sehr groß",
            size: "15 - 30m²",
            price: "60",
            yearlyPrice: "540",
          },
        ],
        note: "* Transport ist NICHT im Preis inbegriffen",
        subscriptionBadge: "Abonnement",
        oneTimeBadge: "Einmalige Dienstleistung",
        subscriptionNote: "Regelmäßige Wartung mit Abonnement",
        oneTimeNote: "Preis für einmalige Dienstleistung",
      },
      packages: {
        title: "Grabpflegeservicepakete",
        subtitle: "Wählen Sie das für Sie am besten geeignete Paket",
        standard: {
          title: "Standard",
          price: "34",
          yearlyPrice: "374",
          description: "Umfassender Grundpflegeservice",
          dressing: "Grab wird geschmückt für:",
          features: ["Weihnachten", "Ostern", "Friedhofsmesse", "Todestage"],
        },
        plus: {
          title: "Plus",
          price: "37",
          yearlyPrice: "407",
          description:
            "Erweiterter Pflegeservice mit zusätzlichen Dekorationen",
          dressing: "Grab wird geschmückt für:",
          features: [
            "Weihnachten",
            "Ostern",
            "Friedhofsmesse",
            "Todestage",
            "Muttertag & Vatertag",
          ],
        },
        premium: {
          title: "Premium",
          price: "44",
          yearlyPrice: "484",
          description: "Komplette Grabpflege mit allen Dekorationsservices",
          dressing: "Grab wird geschmückt für:",
          features: [
            "Weihnachten",
            "Ostern",
            "Friedhofsmesse",
            "Todestage",
            "Muttertag & Vatertag",
            "Geburtstage",
            "Hochzeitstag",
            "Valentinstag",
            "Allerseelen",
          ],
        },
      },
      additionalServices: {
        title: "Zusätzliche Dienstleistungen",
        monthsMind: {
          title: "Monatsgedenkservice",
          description:
            "Wir verstehen, wie schwierig und emotional diese Zeit ist, daher bieten wir einen monatlichen Grabpflegedienst zum Gedenken an. Er wird vor dem Monatsgedenken Ihres Verstorbenen durchgeführt. Es beinhaltet das Entfernen von Steinen, alten Blumen, das Aufräumen des Grabes und das Auffüllen mit einer Auswahl an Torfmoos, Oberboden oder Kies. Schließlich liefern und platzieren wir eine Auswahl frisch geschnittener Blumen in einer Gedenkvasen.",
        },
        graveTending: {
          title: "Grabpflege beinhaltet",
          features: [
            "Trimmen von Gras um den Grabstein und das Grab",
            "Entfernung von Unkraut",
            "Leichte Reinigung von Grabstein, Töpfen und Blumenhaltern",
            "Leichter Schnitt von Heidekraut, Sträuchern, kleinen Bäumen und anderen Pflanzen, falls erforderlich",
            "Entfernung aller Abfälle – alte Blumen, Pflanzen und Blätter usw.",
            "Saisonale Anpflanzung von Grabsträuchern, Zwiebeln und Pflanzen Ihrer Wahl",
          ],
        },
        yearlyServices: {
          title: "Jährliche Grabdienste",
          description:
            "Sie können entscheiden, wie oft Sie das Grab Ihres Verstorbenen pflegen lassen möchten. Sie können aus den folgenden Grabpflegediensten wählen:",
          options: [
            "Monatlich",
            "Zweimonatlich",
            "Vierteljährlich",
            "Alle 6 Monate",
            "Jährlich",
          ],
        },
        floralDelivery: {
          title: "Blumenlieferservice",
          description:
            "Wir bieten die Lieferung und Platzierung von frischen Blumen oder Kranzbeigaben an. Sie können wählen, ob diese bei jedem Besuch oder an besonderen Tagen Ihrer Wahl platziert werden sollen, um Jahrestage, Geburtstage, Mutter- oder Vatertag, Friedhofssonntag zu würdigen. Weihnachtsbeigaben werden einen Tag oder so vor dem Weihnachtstag geliefert.",
          photoNote:
            "Wir fotografieren das Grab vor und nach unserem Pflegedienst. Sie erhalten diese Fotos per Post oder E-Mail.",
        },
        additional: {
          title: "Zusätzliche Dienstleistungen",
          features: [
            "Reinigung und Erneuerung von Grabkies",
            "Erneuerung von Torfmoos oder Oberboden",
            "Betonbasis bedeckt mit Grabkies Ihrer Wahl",
            "Spezialreinigung von Grabstein und Umgebung – Preis nach Inspektion",
            "Grabsteinrestaurierung – Preis nach Inspektion",
            "Neuvergoldung der Epitaphinschrift – Preis nach Inspektion",
          ],
        },
      },
    },
  },
};

// Add types for better type safety
export type Language = "lv" | "en" | "ru" | "de";
export type TranslationKey = keyof typeof translations.lv;
